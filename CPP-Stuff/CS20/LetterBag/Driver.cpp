...
if (b.isSubbagOf(c)) subbagCount++;
if (b < c) lessThanCount++;
if (b > c) greaterThanCount++;
if (b <= c) lessOrEqualCount++;
if (b >= c) greaterOrEqualCount++;
if (b == c) equalCount++;
if (b != c) notEqualCount++;
if (b.getUnion(c) == c.getUnion(b)) unionCount++;
if (b.contains('z') || c.contains('f')) containsCount++;
for (char ch = 'a'; ch <= 'z'; ch++) countAll += b.getCount(ch) + c.getCount(ch);
for (char ch = 'a'; ch <= 'z'; ch++) {
while (b.contains(ch)) {
b.removeOne(ch); removeCount++;
}
while (c.contains(ch)) {
c.removeOne(ch); removeCount++;
}
}
}
